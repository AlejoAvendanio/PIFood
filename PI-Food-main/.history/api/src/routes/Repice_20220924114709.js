...
router.post('/',async (req,res)=>{
    ...
    try{
        const consuApi = await getAll()
        const titleRepeat = consuApi.some(el=>el.title===title)
        if(titleRepeat){
        ...
        res.status(200).json('create recipe')
    }else{
        res.status(400).json('la informacion tiene algun error, revise')
    }}else{
        res.status(400).json('invalid name already exists')
    }
    }catch(error){
        console.log(error)
